...
const MAX_STEPS_ON_ONE_DIRECTION: u32 = 3;
...
fn minimize_heat_loss(lines: Vec<Vec<u32>>) -> u32 {
    ...
    let next_nodes = |node: &Node| {
        let nodes = vec![forward(node), left(node), right(node)];
        // filter out nodes that are out of bound or on the same direction for too long
        nodes
            .into_iter()
            .filter_map(|node| {
